...
export const AdoptionRequestsList: React.FC<AdoptionRequestsListProps> = ({
  postId,
  ownerId,
}) => {
  ...
  const fetchRequests = useCallback(async () => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        const requests: AdoptionRequest[] = [...data];

        // Get all unique requester IDs
        const requesterIds = [
          ...new Set(requests.map((req) => req.requester_id)),
        ];
          ...
          try {
            ...
            if (profilesError && profilesError.code === "42P01") {
              ...
              const userDataPromises = requesterIds.map(async (userId) => {
                const { data: userName, error: userError } = await supabase.rpc(
                  "get_user_name",
                  { user_id: userId }
                );
  ...
  const openRequestDetails = (requestId: number) => {
    ...
    if (!request) {
      toast.error(
        "This adoption request may have been deleted or is no longer available"
      );
      return;
    }
  ...
  if (loading && requests.length === 0) {
    return (
      <div className="my-6 text-center">
        ...
        <p className="mt-4 text-violet-600 font-['Poppins']">
  ...
  if (requests.length === 0) {
    return (
      <div className="my-6 bg-violet-50 rounded-xl p-6 text-center border border-violet-100">
        <p className="text-violet-600 font-['Poppins']">
          No adoption requests yet for this pet.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="my-6">
      <h3 className="text-xl font-semibold text-violet-800 mb-4 font-['Quicksand']">
        Adoption Requests
      </h3>
